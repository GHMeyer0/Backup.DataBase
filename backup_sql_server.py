...
def backup_mmsql():
    ...
    for database in sql_config.DATABASES:
        try:
            ...
            backup_filename = database+'-' + str(now.hour) + str(now.minute) + '.bak'
            aws_s3_object =  database +'/'+ str(now.year) +'/'+ str(now.month) +'/'+ str(now.day) +'/' + backup_filename.replace('.bak','.zip')
            system.create_folder(backup_directory)
            create_sql_file(database, backup_directory, backup_filename)
            run_backup(backup_directory, backup_filename)
            system.compact_file(backup_directory, backup_filename)
            aws.upload_file_to_s3(backup_directory, backup_filename.replace('.bak','.zip'), sql_config.AWS_BUCKET, aws_s3_object)
            mail.send_mail("Backup da base "+ database, "Backup da base " + database + " no servidor " + sql_config.DATABASE_SERVER + " foi realizado.")
        except Exception as e:
            mail.send_logs("Falha no Backup! Favor Verificar!", backup_directory + "log.txt", "Error: %s" % e)


def create_sql_file(database_name, backup_directory, backup_filename):
    file = open(backup_directory + "dbbkp.sql","w+")
    ...
    file.close()
    system.write_log_file("Create SQL file for Backup on: " + backup_directory + "dbbkp.sql", backup_directory)

def run_backup(backup_directory, backup_filename):
    system.write_log_file("Start backup: " + backup_directory + backup_filename, backup_directory)
